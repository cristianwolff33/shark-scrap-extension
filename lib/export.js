...
// WordPress/WooCommerce (i wiele innych CDN-ów) generuje miniaturki jako "<nazwa>-SZERxWYS.<ext>"
// (np. "produkt-300x300.jpg"), a oryginalny, pełnowymiarowy plik zawsze siedzi pod "<nazwa>.<ext>"
// (bez sufiksu) — to konwencja WordPressa (wp_generate_attachment_metadata), bardzo szeroko
// rozpowszechniona poza samym WP.
const THUMBNAIL_SUFFIX_RE = /-\d{2,5}x\d{2,5}(?=\.[a-zA-Z0-9]+(?:[?#].*)?$)/;

/**
 * Zgaduje pełnowymiarowy URL zdjęcia z typowego wzorca nazewnictwa miniaturki WordPress/
 * WooCommerce. To WYŁĄCZNIE heurystyka nazwy — zwrócony URL może nie istnieć (np. jeśli plik
 * naprawdę nazywa się tak, przypadkiem, albo strona nie jest WordPressem). WYWOŁUJĄCY MUSI
 * zweryfikować go realnym fetch() i spaść na oryginalny URL przy niepowodzeniu — nigdy nie ufamy
 * temu w ciemno (patrz fetchImageWithFullSizeUpgrade w sidepanel/main.js). Zwraca "", gdy URL nie
 * pasuje do wzorca (nic do zgadywania).
 */
export function guessFullSizeImageUrl(url) {
  const value = String(url || "");
  if (!value) return "";
  const upgraded = value.replace(THUMBNAIL_SUFFIX_RE, "");
  return upgraded !== value ? upgraded : "";
}
