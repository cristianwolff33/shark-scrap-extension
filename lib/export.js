...
export function normalizeImageTemplate(value) {
  ...
  return `${withProtocol.replace(/\/+$/, "")}/produkty/[sku].[rozszerzenie]`;
}
...
function imageLink(template, sku, sourceUrl) {
  const ext = extensionFromUrl(sourceUrl);
  if (!template) return sourceUrl;
  return template.replaceAll("[sku]", sku || "produkt").replaceAll("[rozszerzenie]", ext);
}
...
export function productsToAdapterRows(products, imageTemplate = "") {
  ...
  return (products || []).map((product, idx) => {
    ...
    productImages(product).forEach((url, imageIdx) => {
      row[`zdj${imageIdx + 1}`] = imageLink(template, sku, url);
    });
    return row;
  });
}
...
export function imagesToZipBlob(images) {
  const files = (images || []).map((img) => ({ name: img.name, content: img.bytes }));
