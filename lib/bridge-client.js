/**
 * @file Cienki klient REST do lokalnego mostu FastAPI (bridge/). Rozszerzenie NIE wykonuje
 * ETL — wysyła tylko konfigurację i steruje istniejącym Scraper Client przez te endpointy.
 */

class BridgeError extends Error {
  constructor(message, status) {
    super(message);
    this.status = status;
  }
}

async function request(baseUrl, path, options = {}) {
  const { fetchImpl = fetch, ...fetchOptions } = options;
  let res;
  try {
    res = await fetchImpl(baseUrl.replace(/\/$/, "") + path, {
      headers: { "Content-Type": "application/json" },
      ...fetchOptions,
    });
  } catch (err) {
    throw new BridgeError(`Nie można połączyć z bridgem pod ${baseUrl} — czy bridge jest uruchomiony? (${err.message})`, 0);
  }
  const text = await res.text();
  const data = text ? JSON.parse(text) : null;
  if (!res.ok) {
    throw new BridgeError(data?.detail || `Bridge zwrócił błąd HTTP ${res.status}`, res.status);
  }
  return data;
}

export function createBridgeClient(baseUrl, fetchImpl = fetch) {
  return {
    health: () => request(baseUrl, "/health", { fetchImpl }),
    createProject: (payload) => request(baseUrl, "/projects", { method: "POST", body: JSON.stringify(payload), fetchImpl }),
    pushConfig: (projectId, config) =>
      request(baseUrl, `/projects/${encodeURIComponent(projectId)}/config`, { method: "POST", body: JSON.stringify(config), fetchImpl }),
    generateAdapter: (projectId) =>
      request(baseUrl, `/projects/${encodeURIComponent(projectId)}/generate-adapter`, { method: "POST", fetchImpl }),
    run: (projectId, exportFormats) =>
      request(baseUrl, `/projects/${encodeURIComponent(projectId)}/run`, {
        method: "POST",
        body: JSON.stringify({ export_formats: exportFormats }),
        fetchImpl,
      }),
    startJob: (projectId, exportFormats) =>
      request(baseUrl, `/projects/${encodeURIComponent(projectId)}/jobs`, {
        method: "POST",
        body: JSON.stringify({ export_formats: exportFormats }),
        fetchImpl,
      }),
    status: (projectId) => request(baseUrl, `/projects/${encodeURIComponent(projectId)}/status`, { fetchImpl }),
    outputs: (projectId) => request(baseUrl, `/projects/${encodeURIComponent(projectId)}/outputs`, { fetchImpl }),
    job: (jobId) => request(baseUrl, `/jobs/${encodeURIComponent(jobId)}`, { fetchImpl }),
    jobLogs: (jobId) => request(baseUrl, `/jobs/${encodeURIComponent(jobId)}/logs`, { fetchImpl }),
    jobOutputs: (jobId) => request(baseUrl, `/jobs/${encodeURIComponent(jobId)}/outputs`, { fetchImpl }),
    listJobs: (projectId = "") => {
      const query = projectId ? `?project_id=${encodeURIComponent(projectId)}` : "";
      return request(baseUrl, `/jobs${query}`, { fetchImpl });
    },
    listProjects: () => request(baseUrl, "/projects", { fetchImpl }),
    // Fallback AI (OpenAI) NIE idzie już przez bridge — woła się bezpośrednio z przeglądarki,
    // patrz lib/openai-client.js + lib/storage.js (loadOpenAiSettings/saveOpenAiSettings).
  };
}

export { BridgeError };
