...
// Działa zarówno na już zhumanizowanym tekście ze schema.org (patrz humanizeAvailability w
// lib/jsonld.js, np. "Niedostępny"/"Wyprzedany"/"Wycofany"), jak i na surowym tekście wykrytym w
// DOM (".stock-status"/".availability" itp., patrz FIELD_SELECTORS w lib/dom-heuristics.js) — oba
// trafiają do tego samego pola "availability" po scaleniu (mergeFieldSources), więc jeden wzorzec
// wystarcza niezależnie od źródła. Celowo NIE łapiemy "ograniczona dostępność"/"na zamówienie"/
// "przedsprzedaż" — to wciąż produkty, które można kupić, tylko z zastrzeżeniem.
const UNAVAILABLE_AVAILABILITY_RE =
  /(niedostępn|wyprzedan|wycofan|brak w magazynie|brak towaru|brak na stanie|chwilowo brak|out[\s-]?of[\s-]?stock|sold[\s-]?out|unavailable|discontinued|no longer available)/i;

/**
 * Czy scalone pola produktu wskazują, że jest NIEDOSTĘPNY (a nie tylko "ograniczona dostępność"
 * czy "na zamówienie", które wciąż da się kupić) — user chciał, żeby taki produkt był POMIJANY w
 * wynikach skanu, nie tylko oznaczony. Brak jakiejkolwiek informacji o dostępności traktujemy jako
 * DOSTĘPNY (domyślnie NIE pomijamy) — lepiej pokazać produkt bez pewności niż fałszywie zgubić
 * taki, którego strona po prostu nie oznacza statusu dostępności w żaden wykrywalny sposób.
 * @param {Record<string, any>} merged - scalone pola produktu (po mergeFieldSources)
 * @returns {boolean}
 */
export function isProductUnavailable(merged) {
  const text = merged?.availability?.value;
  if (!text) return false;
  return UNAVAILABLE_AVAILABILITY_RE.test(String(text));
}
