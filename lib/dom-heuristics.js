...
/**
 * Zbiera WSZYSTKIE (nie tylko pierwszy) adresy zdjęć z JEDNEGO, KONKRETNEGO kontenera galerii —
 * tego samego, który wygrał detekcję (patrz firstMatch poniżej) — używane przez przycisk
 * "Zdjęcia" w prostym trybie (sidepanel/main.js), żeby pobrać cały zestaw zdjęć produktu, nie
 * tylko podgląd. Deduplikowane, bez twardego limitu tutaj (limit bezpieczeństwa jest w main.js
 * na poziomie całego skanu).
 *
 * UWAGA: wcześniejsza wersja przeszukiwała TYMI SAMYMI selektorami galerii CAŁY `root`
 * (dokument), a nie tylko wygrany kontener — ogólne selektory takie jak "figure img"/"picture
 * img" łapały wtedy też: logo w nagłówku (często owinięte w `<picture>` pod WebP/AVIF) oraz
 * zdjęcia z zupełnie innych sekcji strony (karuzele "Ostatnio przeglądane"/"Bestsellery" itp.,
 * jeśli akurat pasowały do któregoś z selektorów). User zgłosił oba objawy naraz: czasem brak
 * zdjęć produktu, czasem zamiast nich logo strony. Teraz zbieramy zdjęcia WYŁĄCZNIE z najbliższego
 * przodka pasującego do selektora KONTENERA (nie samego "img"), który zawiera zwycięski element.
 * @param {Document|Element} root
 */
export function allImageUrls(root) {
  ...
  const containerSelector = gallery.selector.replace(/\s+img$/, "");
  let container = root;
  try {
    container = gallery.el.closest?.(containerSelector) || root;
  } catch {
    container = root;
  }
  ...
  let imgs;
  try {
    imgs = container.querySelectorAll ? container.querySelectorAll("img") : [];
  } catch {
    imgs = [];
  }
