...
export function scoreGroupDescriptor(d) {
  ...
  // Karuzele "Bestsellery"/"Polecane dla Ciebie"/"Klienci kupili też" nad właściwą listą mają
  // markup łudząco podobny do prawdziwych kart produktu (cena+zdjęcie+link) — user zgłosił, że
  // wtyczka scrapowała produkty z takich karuzel razem z prawdziwym listingiem. `looksLikeCarousel`
  // ustawia content/detector.js na podstawie klasy biblioteki karuzeli/nazwy widgetu na elemencie
  // lub bliskim przodku (patrz CAROUSEL_OR_RECOMMENDATION_CLASS_RE) — twardy odrzut, bez wyjątków,
  // tak samo jak przy braku ceny/zdjęcia: to nigdy nie jest właściwy katalog produktów.
  if (d.looksLikeCarousel) return -Infinity;
...
// Klasy bibliotek karuzeli (JS slidery) ORAZ nazwy semantyczne widgetów rekomendacji/bestsellerów
// — w PL i EN, z wariantami z myślnikiem (typowa konwencja nazewnictwa klas CSS). Dopasowanie na
// SAMYM elemencie karty LUB do kilku poziomów w górę drzewa DOM (patrz content/detector.js:
// looksLikeCarouselOrRecommendationWidget) oznacza "to nie jest właściwy listing, to widget".
export const CAROUSEL_OR_RECOMMENDATION_CLASS_RE =
  /(carousel|slider|swiper|splide|slick|owl-carousel|glide\.js|flickity|keen-slider|tiny-slider|siema-?slider|best-?seller|recommend|recently-?viewed|cross-?sell|up-?sell|related-?product|also-?bought|frequently-?bought|you-?may-?also-?like|polecan|bestseller|rekomendowan|podobne-?produkt|ostatnio-?ogladan|moze-?cie-?zainteresowac|czesto-?kupowane|klienci-?kupili)/i;
